...
function initialiseVenueLocationTabs() {
  ...
  function activateVenueTab(key, focusTab = false) {
    tabs.forEach(tab => {
      const active = tab.dataset.venueTab === key;
    ...
    panels.forEach(panel => {
      const active = panel.dataset.venuePanel === key;
  ...
  tabs.forEach((tab, index) => {
    ...
    tab.addEventListener("keydown", event => {
      ...
      activateVenueTab(tabs[nextIndex].dataset.venueTab, true);
    });
  });

  activateVenueTab(tabs.find(tab => tab.classList.contains("is-active"))?.dataset.venueTab || "wedding");
}
