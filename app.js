...
const ENTOURAGE_SUPABASE_FIELDS = [
  { column: "entourage_parents_groom", slug: "parents-groom", role: "Parents of the Groom" },
  ...
  { column: "entourage_groomsmen", slug: "groomsmen", role: "Groomsmen" },
  { column: "entourage_bridesmaids", slug: "bridesmaids", role: "Bridesmaids" },
  { column: "entourage_ring_bearer", slug: "ring-bearer", role: "Ring Bearer" },
  { column: "entourage_coin_bearer", slug: "coin-bearer", role: "Coin Bearer" },
  { column: "entourage_bible_bearer", slug: "bible-bearer", role: "Bible Bearer" },
  { column: "entourage_flower_girls", slug: "flower-girls", role: "Flower Girls" }
];

function entourageNamesForSlug(items, slug) {
  return (Array.isArray(items) ? items : [])
    .filter(item => entourageRoleSlug(item.role) === slug)
    .flatMap(item => normaliseEntourageNames(item.names));
}

function entourageToSupabaseColumns(items) {
  return Object.fromEntries(ENTOURAGE_SUPABASE_FIELDS.map(field => [
    field.column,
    entourageNamesForSlug(items, field.slug)
  ]));
}

function rowHasSeparateEntourageColumns(row) {
  return Boolean(row) && ENTOURAGE_SUPABASE_FIELDS.some(field =>
    Object.prototype.hasOwnProperty.call(row, field.column)
  );
}

function entourageFromSeparateColumns(row) {
  return ENTOURAGE_SUPABASE_FIELDS
    .map(field => {
      const value = row?.[field.column];
      const names = Array.isArray(value)
        ? value.map(name => String(name).trim()).filter(Boolean)
        : [];
      return names.length ? { role: field.role, names } : null;
    })
    .filter(Boolean);
}
...
function weddingToRow(wedding) {
  return {
    ...
    ...entourageToSupabaseColumns(wedding.entourage),
...
function rowToWedding(row) {
  ...
  return {
    ...
    entourage: (() => {
      ...
      if (Array.isArray(row.entourage) && row.entourage.length) return row.entourage;
      return clone(defaultState.wedding.entourage);
    })(),
...
function parseEntourageAdminNames(value) {
  return String(value || "")
    .split(/\r?\n|;/)
    .map(name => name.trim())
    .filter(Boolean);
...
function entourageAdminValue(items, roleSlug) {
  return (Array.isArray(items) ? items : [])
    .filter(item => entourageRoleSlug(item.role) === roleSlug)
    .flatMap(item => normaliseEntourageNames(item.names))
    .join("\n");
}

function buildEntourageAdminItem(role, fieldId) {
  const field = byId(fieldId);
  const names = parseEntourageAdminNames(field?.value || "");
...
function renderPrimarySponsors(item) {
  if (!item) return "";
  const names = normaliseEntourageNames(item.names);
  if (!names.length) return "";

  const midpoint = Math.ceil(names.length / 2);
  const columns = [names.slice(0, midpoint), names.slice(midpoint)].filter(column => column.length);

  return `
    <article class="entourage-group entourage-role-principal role-primary-sponsor entourage-primary-sponsors">
      <div class="entourage-group-heading">
        <p>${escapeHtml(item.role || "Primary Sponsors")}</p>
      </div>
      <div class="entourage-primary-columns ${columns.length === 1 ? "is-single-column" : ""}">
        ${columns.map(column => `
          <div class="entourage-primary-column">
            ${column.map(name => `
              <div class="entourage-member">
                <span>${escapeHtml(name)}</span>
              </div>
...
function renderEntourage() {
  ...
  items.forEach((item) => {
    ...
    if (!grouped.has(slug)) {
      grouped.set(slug, { ...item, names: normaliseEntourageNames(item.names) });
      return;
    }

    const existing = grouped.get(slug);
    existing.names = [...normaliseEntourageNames(existing.names), ...normaliseEntourageNames(item.names)];
  });

  const get = (slug) => grouped.get(slug) || null;
  const row = (className, groups) => {
    const markup = groups.filter(Boolean).map(group => renderEntourageGroup(group)).join("");
    return markup ? `<div class="entourage-layout-row ${className}">${markup}</div>` : "";
  };
  ...
  const attendantsRow = attendantGroups.length
    ? `<div class="entourage-subsection entourage-attendants-subsection">
        <h3>Groomsmen &amp; Bridesmaids</h3>
        <div class="entourage-layout-row entourage-attendants-row">
          ${attendantGroups.map(group => renderEntourageGroup(group)).join("")}
        </div>
      </div>`
    : "";
  const bearersRow = row("entourage-bearers-row", [get("ring-bearer"), get("coin-bearer"), get("bible-bearer")]);
  const flowerRow = row("entourage-flower-row", [get("flower-girls")]);

  const otherMarkup = otherItems.length
    ? `<div class="entourage-layout-row entourage-other-row">
        ${otherItems.map(item => renderEntourageGroup(item)).join("")}
      </div>`
    : "";
...
function populateSettingsForm() {
  ...
  byId("settingParentsGroom").value = entourageAdminValue(adminEntourage, "parents-groom");
  byId("settingParentsBride").value = entourageAdminValue(adminEntourage, "parents-bride");
  byId("settingPrimarySponsors").value = entourageAdminValue(adminEntourage, "primary-sponsor");
  byId("settingMaidHonor").value = entourageAdminValue(adminEntourage, "maid-of-honor");
  byId("settingBestMen").value = entourageAdminValue(adminEntourage, "best-men");
  byId("settingVeil").value = entourageAdminValue(adminEntourage, "veil");
  byId("settingCord").value = entourageAdminValue(adminEntourage, "cord");
  byId("settingCandle").value = entourageAdminValue(adminEntourage, "candle");
  byId("settingGroomsmen").value = entourageAdminValue(adminEntourage, "groomsmen");
  byId("settingBridesmaids").value = entourageAdminValue(adminEntourage, "bridesmaids");
  byId("settingRingBearer").value = entourageAdminValue(adminEntourage, "ring-bearer");
  byId("settingCoinBearer").value = entourageAdminValue(adminEntourage, "coin-bearer");
  byId("settingBibleBearer").value = entourageAdminValue(adminEntourage, "bible-bearer");
  byId("settingFlowerGirls").value = entourageAdminValue(adminEntourage, "flower-girls");
...
byId("weddingDetailsForm").addEventListener("submit", async e => {
  ...
  const entourage = [
    buildEntourageAdminItem("Parents of the Groom", "settingParentsGroom"),
    ...
    buildEntourageAdminItem("Groomsmen", "settingGroomsmen"),
    buildEntourageAdminItem("Bridesmaids", "settingBridesmaids"),
    buildEntourageAdminItem("Ring Bearer", "settingRingBearer"),
    buildEntourageAdminItem("Coin Bearer", "settingCoinBearer"),
    buildEntourageAdminItem("Bible Bearer", "settingBibleBearer"),
    buildEntourageAdminItem("Flower Girls", "settingFlowerGirls")
  ].filter(Boolean);
