...
const defaultState = {
  wedding: {
    ...
    dressMotif: "We would love our guests to dress in elegant tones inspired by our wedding palette. Emerald green, gold, champagne, and soft ivory accents are especially welcome.",
    themeColors: {
      olive: "#0b5d46",
      burgundy: "#0f7557",
      cream: "#f7f1e3",
...
function getThemeColors(theme = state.wedding.themeColors) {
  ...
  // V21 palette migration: older releases stored olive/burgundy defaults in
  // localStorage/Supabase. Upgrade those known legacy combinations so they
  // cannot overwrite the emerald + gold theme after page load.
  const legacyPrimary = new Set(["#4f5130", "#5f6f3a"]);
  const legacyAccent = new Set(["#73333a", "#7b2438"]);
  const legacyCream = new Set(["#ebe6da", "#f5f1e7"]);
  const legacyGold = new Set(["#bca36e", "#b08d57"]);
  if (legacyPrimary.has(colors.olive)) colors.olive = defaults.olive;
  if (legacyAccent.has(colors.burgundy)) colors.burgundy = defaults.burgundy;
  if (legacyCream.has(colors.cream)) colors.cream = defaults.cream;
  if (legacyGold.has(colors.gold)) colors.gold = defaults.gold;
